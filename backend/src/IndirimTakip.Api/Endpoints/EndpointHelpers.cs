...
internal static class EndpointHelpers
{
    ...
    // BuildInfoPage ile aynı kart, ama düğme bağlantının kendisine POST gönderiyor.
    // E-posta bağlantı tarayıcıları (Gmail'inki dahil) bulduğu her bağlantıyı
    // açıyor; durumu GET ile değiştiren bir bağlantı kimsenin istemediği
    // abonelikleri onaylatıyordu. Tarayıcılar form göndermiyor, değişiklik
    // yalnızca gerçek tıklamada oluyor.
    // actionPath adresteki jetonu taşıyor, yani ziyaretçi girdisi: çağıran taraf
    // yol parçası olarak kaçırıyor, burada da HTML için kodlanıyor.
            ...
            <body style="margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;box-sizing:border-box;background:#fafaf9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
              <div style="max-width:420px;width:100%;background:#ffffff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.08);padding:40px 32px;text-align:center;">
                ...
                <form method="post" action="{action}" style="margin:0 0 16px;">
                  <button type="submit" style="display:inline-block;border:0;cursor:pointer;background:#059669;color:#fff;font-weight:700;font-size:15px;padding:13px 32px;border-radius:9999px;">{buttonLabel}</button>
                </form>
                <a href="{frontendBaseUrl}" style="color:#78716c;font-size:13px;">Siteye dön</a>
              </div>
            </body>
            </html>
            """;
